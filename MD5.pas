...
function MD5DigestFromString(const A: string): TMD5Digest;
...
function _F(x, y, z: UINT4): UINT4; inline;
begin
 Result := (((x) and (y)) or ((not x) and (z)));
end;

function _G(x, y, z: UINT4): UINT4; inline;
begin
 Result := (((x) and (z)) or ((y) and (not z)));
end;

function _H(x, y, z: UINT4): UINT4;inline;
begin
 Result := ((x) xor (y) xor (z));
end;

function _I(x, y, z: UINT4): UINT4;inline;
begin
 Result := ((y) xor ((x) or ( not z)));
end;

function ROTATE_LEFT(x, n: UINT4): UINT4;inline;
begin
 Result := (((x) shl (n)) or ((x) shr (32-(n))));
end;

procedure FF(var a: UINT4; b, c, d, x, s, ac: UINT4); inline;
begin
  a := a + _F(b, c, d) + x + ac;
  a := ROTATE_LEFT (a, s);
  a := a + b;
...
procedure GG(var a: UINT4; b, c, d, x, s, ac: UINT4); inline;
begin
 a := a + _G(b, c, d) + x + ac;
 a := ROTATE_LEFT(a, s);
 a := a + b;
...
procedure HH(var a: UINT4; b, c, d, x, s, ac: UINT4);inline;
begin
 a := a + _H(b, c, d) + x + ac;
 a := ROTATE_LEFT(a, s);
 a := a + b;
...
procedure II(var a: UINT4; b, c, d, x, s, ac: UINT4); inline;
begin
 a := a + _I(b, c, d) + x + ac;
 a := ROTATE_LEFT(a, s);
 a := a + b;
...
begin
  ...
  for i := 0 to 15 do
    Result.v[i] := ('$'+A.Substring(i*2,2)).ToInteger;
end;

function TMD5Digest.ToString: string;
begin
  result := MD5DigestToStr(self);
end;
